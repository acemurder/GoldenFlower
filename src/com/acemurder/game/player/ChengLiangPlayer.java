...
public class ChengLiangPlayer implements Player {
    ...
    public int bet(int time, int round, int lastPerson, int moneyOnDesk, int moneyYouNeedToPayLeast, List<Poker> pokers) {
        ...
        if(time<1500){//经过多方实验1500左右王尼玛必死，接着干掉弃牌狗。
        if (isSameColor(pokers) )
            return 3 * moneyYouNeedToPayLeast;//拿到对子以上就开干。
        ...
        if (isStraight(pokers))
            return 3 * moneyYouNeedToPayLeast;
        return 0;}else {//王尼玛应该死了，妈的不要怂就是干。
           return 3 * moneyYouNeedToPayLeast;
      }
    }
